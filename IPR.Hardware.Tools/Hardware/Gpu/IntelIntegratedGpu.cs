...
namespace IPR.Hardware.Tools.Hardware.Gpu
{
    internal class IntelIntegratedGpu : GenericGpu
    {
        ...
        public IntelIntegratedGpu(CPU.IntelCpu intelCpu, string deviceId, D3DDisplayDevice.D3DDeviceInfo deviceInfo)
                   ...
                   new Identifier("gpu-intel-integrated", deviceId.ToString(CultureInfo.InvariantCulture)))
        {
            ...
            {
                _dedicatedMemoryUsage = new Sensor("D3D Dedicated Memory Used", memorySensorIndex++, SensorType.SmallData, this);
            }

            _sharedMemoryUsage = new Sensor("D3D Shared Memory Used", memorySensorIndex++, SensorType.SmallData, this);

            if (Ring0.Instance.ReadMsr(MSR_RAPL_POWER_UNIT, out uint eax, out uint _))
            {
                ...
                if (_energyUnitMultiplier != 0)
                {
                    ...
                    _powerSensor = new Sensor("GPU Power", 0, SensorType.Power, this);
            ...
            {
                _nodeUsage[node.Id] = new Sensor(node.Name, nodeSensorIndex++, SensorType.Load, this);
                _nodeUsage[node.Id].AddRangeParameter(0, 100);
        ...
        public override void Update()
        {
            if (D3DDisplayDevice.GetDeviceInfoByIdentifier(_deviceId, out D3DDisplayDevice.D3DDeviceInfo deviceInfo))
            {
                ...
                if (_powerSensor != null && Ring0.Instance.ReadMsr(MSR_PP1_ENERGY_STATUS, out uint eax, out uint _))
                {
